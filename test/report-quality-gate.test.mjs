...
test('CLI binds the receipt to bytes on disk',()=>{
  ...
  try {
    ...
    const run=()=>spawnSync(process.execPath,[resolve('scripts/quality-gate.mjs'),pdf,json],{encoding:'utf8',timeout:10000});
...
test('CLI --profile selects the assurance level and rejects an unknown one',()=>{
  ...
  try {
    ...
    const run=(...extra)=>spawnSync(process.execPath,[resolve('scripts/quality-gate.mjs'),pdf,json,...extra],{encoding:'utf8',timeout:10000});
