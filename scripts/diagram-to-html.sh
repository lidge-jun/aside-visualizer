...
#   diagram-to-html.sh mermaid diagram.mmd /tmp/dev-visualizer-diagrams/out.html
#   diagram-to-html.sh chartjs chart.js
#
# When output-file is omitted, writes to /tmp/dev-visualizer-diagrams/<type>-<timestamp>-<pid>-<random>.html

set -euo pipefail

TYPE="${1:?Usage: diagram-to-html.sh <type> <input-file|-> [output-file]}"
INPUT="${2:?Usage: diagram-to-html.sh <type> <input-file|-> [output-file]}"
OUTDIR="/tmp/dev-visualizer-diagrams"
mkdir -p "$OUTDIR"
